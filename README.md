...
## 4. 学习收获
1. 文献检索和阅读能力
2. 学习新的领域：计算机视觉（图像分类）、篡改检测、数据血缘、深度学习的可解释性
3. 锻炼了技能：SQL, PyTorch, Python

在进行这个项目的过程中，我收获并学习到了很多的技能和知识。在刚刚接触项目上手时，通过查询百科了解到分类的对象是“病媒生物”，需要进行的任务在机器学习、深度学习中归类为计算机视觉下的图像分类任务。随后，以“病媒（vector）”、“害虫（pest）”、“机器学习（machine learning）”、“图像分类（image classification）”为关键词在Google Scholar上搜索相关文献，进行综述并了解目前相关领域的技术路线。之后学习了使用PyTorch进行模型训练，尝试复刻iNaturalist Competition 2021冠军团队的技术路线，但碍于硬件限制转而微调了一个预训练的小模型。考虑到模型分类会存在错误，以及真实应用场景下对识别出的物种相关信息的需求，尝试使用SQLite构建了一个包含物种信息的数据库框架，并将其与模型分类模块进行拼接以实现分类—查询的一键式流程。在这一过程中，我学习了数据库和机器学习的基本知识，掌握了实际任务中SQL和PyTorch的基本应用；在数据搜集等过程中编写脚本也提升了我使用Python进行编程解决实际任务的能力。此外，为了验证训练数据的可信度，我还去了解了数据血缘和篡改检测的相关知识，笔记：[数据血缘](https://www.notion.so/Data-Lineage-2347b3784acb8049ba75f0b5319e3cb2?source=copy_link)、[篡改检测](https://www.notion.so/2377b3784acb80ac8c77c66c79ede424?source=copy_link)。
   
值得一提的是，在查找论文、了解新的领域的过程中，我也找到了一些自己颇感兴趣的东西。在Mora, Camilo, et al.所著的 "How many species are there on Earth and in the ocean?."中使用回归将问题简化，使用已知探索未知的思路。在学习SQL和深度学习的过程中，我也对课内专业知识的应用有了更深的理解，例如在SQL中提高查询效率所涉及到的计算机原理，以及深度学习中所涉及的各种微积分、统计和线性代数知识。同时，通过这段时间对深度学习的认识，我觉得可解释性是一个十分有趣且重要的方向，对深度学习可解释性的探索或许能启发我们学到一些新的东西。

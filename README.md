...
### 进行分类
分类相关代码在predict.py中。其中的函数/API包括：
 - load_model(): 加载模型并返回。
...
### 整合流程
通过调用前面封装好的API，workwork.py可以实现对于给定图片，输入路径后返回识别结果及物种相关信息的操作。
